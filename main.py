...
async def show_today_orders(ctx):
    """!tdy：顯示取貨日期 = 今日嘅所有訂單內容（cakes + quantities）。"""
    try:
        now = datetime.now(HK_TZ)
        yymmdd = now.strftime("%y%m%d")  # FORCE TODAY ONLY
        ...
        # Consolidate all items for today
        all_items = {}
        for order in orders:
        ...
        msg_lines = [f"📋 **Today's Orders** ({now.strftime('%Y-%m-%d')})"]
        msg_lines.append("=" * 50)
        ...
        for guild in bot.guilds:
            for channel in guild.text_channels:
                if channel.name.lower() == "cake":
                    ...
                    await send_reply("✅ Today's orders sent to #cake.")
                    return
...
async def check_order_details(ctx, date_arg: str):
    """
    !d yymmdd：顯示指定日期嘅訂單詳情（author, phone, delivery, remark）
    !d yymm：顯示指定月份嘅訂單詳情
    ...
    try:
        ...
        if REMINDER_CHANNEL_ID > 0:
            ...
            await send_reply(f"❌ REMINDER_CHANNEL_ID not set.")
...
async def check_order_contents(ctx, date_arg: str):
    ...
    try:
        ...
        # Send to BOT_COMMAND_CHANNEL
        full_text = "\n".join(msg_lines)
        await send_reply(f"✅ Orders:\n\n{full_text}")
